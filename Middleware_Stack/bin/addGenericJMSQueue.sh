...
while read line1 ; do
	...
	providerName=`echo $line2 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	name=`echo $line3 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	description=`echo $line4 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	jndiName=`echo $line5 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	extJNDI=`echo $line6 | awk -F"=" '{print $2}' | sed 's/\"//g'`

        arg_list=("$scope" "$providerName" "$name" "$description" "$jndiName" "$extJNDI")
