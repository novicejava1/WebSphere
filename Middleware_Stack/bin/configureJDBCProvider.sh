...
while read line1 ; do
	...
	dbType=`echo $line2 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	providerType=`echo $line3 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	impleType=`echo $line4 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	name=`echo $line5 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	classpath=`echo $line6 | awk -F"=" '{print $2}' | sed 's/\"//g'`
	...
	arg_list=("$scope" "$dbType" "$providerType" "$impleType" "$name" "$classpath")
